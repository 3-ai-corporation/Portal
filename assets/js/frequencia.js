...
/* (function() { //função de escopo global, todo o js é uma função
	...
	frequenciaModule.controller('frequenciacontroller', function() {
		for(var i=0;i<5;i++){
			dias[i].lst_alunos = alunos;
		}
		this.objetos = dias[2];
	});
	
    frequenciaModule.controller ('dataController', function($dateSelected) {
		for(var i=0;i<5;i++){
			lstdias[i].lst_alunos = alunos;
		}
		
		for(var i=0; i < diasLetivos.lstDias.length; i++) {
			if(dateSelected === diasLetivos.lstdias[i].data) {
				this.objetos = diasLetivos.lstdias[i];
			}
		}
	});
...
var dias = [];
var temps = [ [1,2], [1], [1,2,3], [1,2], [1]];
var dateList = ["06/09/2014","10/09/2014","12/09/2014","14/09/2014","16/09/2014"];
var i;
var count = 0;
for(i=1; i<6; i++){
	dias[i-1] = {id:i+"092014", data: datelist[i],lst_alunos: [], tempos_dia:temps[count]};
	count++;
	if(count>temps.lenght-1){
		count = 0;
	};
};
	

var diasLetivos = {idMateria: 1, idProfessor: 200872, lstDias: dias};
