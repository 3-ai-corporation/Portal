...
<article class="quadroNotas" ng-controller="AppController as appCtrl">
	...
	<!-- <table id="rowAluno" class="table table-bordered"  ng-controller="ColumnController as colCtrl">
		...
		<table id="rowAluno" class="table table-bordered"  ng-controller="ColumnController">
		<tr class="warning">
			<th>#</th>
			<th>RA</th>
			<th>Nome</th>
			<th ng-repeat="notas in notasList" ng-click="setTooltip({{av.title}})"><a class="tooltips">{{av.title}}<span ng-show="isSet({{av.title}})"><input ng-model="av.value" onkeypress="return isNumberKey(event)" type="text" maxlength="4"></span></th>
		<!--	<th ng-click="setColumn()"><a class="btnNewCollumn"> <img class="imgMenu" src="assets/img/adicionar.png" style="height: 24px; width:24px"/> </a></th>
			<th ng-click="unsetColumn()"><a class="btnDelCollumn"> <img class="imgMenu" src="assets/img/minus.png" style="height: 24px; width:24px"/> </a></th> -->
			<th>MP</th>
			<th>Paralela</th>
			<th>MF</th>
		</tr>
