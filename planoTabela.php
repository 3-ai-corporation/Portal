<table class = "table table-striped table-hover table-condensed" ng-controller = "GridController as grid">
	...
	<tbody>
		...
		<tr ng-repeat = "aula in grid.aulas" ng-show="(aula.geminada < 2) || aula.visible" ng-class="(aula.geminada > 1? 'row-collapsed':'row-showed')">
			...
			<td><input class="table-text" onblur="AtualizarPrevisto();" onkeydown="UnFocusOnKeyDown(event, this)" type="text" ng-model="aula.conteudo_previsto" ng-change="grid.updatePrevisto(aula)"></td>
			<td><input class="table-text" onblur="AtualizarAplicado();" onkeydown="UnFocusOnKeyDown(event, this)" type="text" ng-model="aula.conteudo_aplicado" ng-change="grid.updateAplicado(aula)"></td>
